...
socket.on('notifyUser', function (user) {
      var me = $('#user').val()
        if (user != me) {
               $('#notifyUser').text(user + ' is typing ...')  } // 10 seconds after typing stops, set the notify text to an empty string  setTimeout(function () { $('#notifyUser').text('') }, 10000)})

// when does the document.ready() function get executed?...................
$(document).ready(function () {
      var name = makeid()
        $('#user').val(name)  // emit a chatMessage event from the System along with a message   socket.emit('chatMessage', 'System', '<b>' + name + '</b> has joined the discussion')})
